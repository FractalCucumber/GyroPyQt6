...
class SecondThread(QtCore.QThread):
    ...
    def __init__(self, gyro_number, READ_INTERVAL_MS, logger_name: str = ''):
        ...
        self.bourder.shape = (2, 1)
        # self.bourder[0, 0] = 22
        # self.bourder[1] = 2722
        # self.bourder = self.get_new_bourder(self.bourder, 1000)  # можно здесь задерржки учесть
        # print(self.bourder)
...
# -------------------------------------------------------------------------------------------------
    ...
    def make_fft_frame_generator(self):
        ...
        while True:
            ...
            gyro_list=self.time_data[:, 1::self.pack_len]
            # print(not self.flag_send)
            # print(not flag_frame_start)
            # print(not self.flag_send and not flag_frame_start)
            # print("=====")
            if not self.flag_send and not flag_frame_start: # пусть срабатывает только 1 разpr
                print("in")
                flag_frame_start = True
                self.bourder[0,0] = self.pack_num  # frame start
            elif flag_frame_start and self.flag_send:
                print("out")
                flag_frame_start = False
                self.bourder[1,0] = self.pack_num  # frame end
                print(self.bourder)
                self.bourder = self.bourder + self.delays
                print(self.bourder)
                ...
                print(self.bourder)
                print(self.bourder.shape)
                if self.bourder[1]:
                    for i in range(self.GYRO_NUMBER):
                        [freq, amp, d_phase] = get_fft_data(
                            # gyro=gyro_list[bourder[0]:bourder[1], i],
                            gyro=(gyro_list[self.bourder[0,0]:self.bourder[1,0], i] - self.amp_shift[i]),  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            # добавил сюда учет смещения
                            encoder=encoder[self.bourder[0,0]:self.bourder[1,0]] * self.k_amp[i],
                            fs=self.fs)
...
# -------------------------------------------------------------------------------------------------
    ...
    def fft_from_file_median(self, file_list: list):
        ...
        filter_list: list = [
            (np.ones(filter_len) / filter_len * 1.5).astype(np.float32),  # const_filter = 
            (custom_g_filter(len=25, k=0.0075) * 1).astype(np.float32)  # g_filter = 
        ]
        # filter_list.append(const_filter)
        # filter_list.append(g_filter)
...
# -------------------------------------------------------------------------------------------------
    ...
    def fft_for_file(self, filename: str, filter_list: list, threshold: int = 5500):
        ...
        for filter in filter_list:
            bool_arr = np.convolve(bool_arr, filter, 'same')
        ...
        flag_first = True
        start_arr += self.delays[0]
        end_arr += self.delays[1]
        # start_arr[0] = 12999
        print(end_arr)
        [start_arr, end_arr] = self.get_new_bourder(
            np.array([start_arr[:rows_count], end_arr[:rows_count]]), self.fs)  # можно векторно округлить
        print(end_arr)
        for i in range(rows_count):
            ...
            if end_arr[i]:
                [freq, amp, d_phase] = get_fft_data(
                    gyro=time_data[start_arr[0]:end_arr[1], 1-1],
                    encoder=time_data[start_arr[0]:end_arr[1], 2-1] * self.k_amp[0],
                    fs=self.fs)
                [freq, amp, d_phase, tau] = self.fft_normalisation(
                    freq, amp, d_phase)
...
# -------------------------------------------------------------------------------------------------
    ...
    @staticmethod
    def get_new_bourder(bourders: np.ndarray, fs: int):
        ...
        # print(bourder)
        # print(bourder[1, :] - bourder[0, :])
        # print((bourder[1, :] - bourder[0, :]) >=fs)
        # print(np.greater_equal((bourder[1, :] - bourder[0, :]), fs))
