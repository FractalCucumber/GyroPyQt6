...
class MyThread(QtCore.QThread):
    ...
    def fft_for_file(self, filename: str, threshold: int = 6000):
        ...
        #         self.filename[1], 'w') as file:
        #     np.savetxt(file, arr, delimiter='\t', fmt='%.3f')
        self.logger.info("2")
        ...
        arr = np.convolve(arr, self.custom_filter(55, 0.0065)*1.45, 'same') # работает
        ...
        real_start = np.where(np.diff(start) > self.fs)[0]
        start_arr = np.insert(start[real_start + 1], 0, start[0])

        real_end = np.where(np.diff(end) > self.fs)[0]
        ...
        for i in range(len(end_arr)):
            ...
            bourder = self.new_bourder([start_arr[i], end_arr[i]])
            [freq, amp, d_phase, tau] = self.fft_data(
