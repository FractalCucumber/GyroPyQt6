...
//        enc_angl_gr точно не 4 или 9 раз суммируется?
		if (flag_vibration) ////////////////////////////////////////////////////////////////////
		{
		FrqNmb = 40000/FrqVibr; // вот здесь ошибка
		iVibro++;
         ...
         // почему vel_angle считается в конце? Тогда получается, что мы с запаздыванием работаем
         // 
            // Current Angle Velocity calculation  4kHz freq. 
					angle_sm += enc_angl_gr; // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                 angle = angle_sm;
                 angle_sm = 0.0; 
...
//          	 	 if (shift_anglMot >= 2*pi) shift_anglMot = shift_anglMot - 2*pi;
				...
				// -- PI  regulator    -----      Amplitude Stabilization  -------------------------------------
						 if (flag_vibration)  // Start vibration 
						  {                   
							//FrqNmb = 4000/FrqVibr; // вот здесь ошибка
							//iVibro++; 
							/*if (iVibro > FrqNmb) // в одном периоде FrqNmb интераций
							{
								err_amp = (SetVel*5 - max_vel)/(SetVel*5);
								integral_amp += err_amp;
...
// Current Angle Velocity calculation  4kHz freq. 
	/*				angle_sm += enc_angl_gr; // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           ...
           else {angle_sm += enc_angl_gr;} // точно не 4 раза сработает, а 5?
