...
# rx222: bytes = b'\x72\xFF\xFF\xFF\x00\x00\x02\xFF\xFF\xFF\x00\x02\x09\x27\x72\x07\x02\x0F\x02\x01\x21\x25\x00\x00\x02\x00\x00\x00\x03\x00\x27'
...
t = time()
package_num = 0
time_data = np.ndarray((1, 5), dtype=np.int32)
for f in range(101):
    ...
    time_data[package_num:, 0] = np.arange(package_num + 1, expand + package_num + 1)
    time_data[package_num:, 1:] = (np.einsum("ijk,jk->ij", array_r, b) / 256)
    package_num += expand
print(time() - t)
# print(time_data)
# print(start)
...
# for k in range(1):
#     i = rx222.find(0x72) + 1
#     a2rr = np.frombuffer(rx222[i:], dtype=np.uint8)
#     # print(a2rr)
#     # start = np.where(a2rr == 5)[0]
#     start = np.where((a2rr[:-1] == 0x27) & (a2rr[1:] == 0x72))[0] + 2
#     start = np.insert(start, 0, 0)  # i
#     start = start[np.where(np.diff(start) == 14)[0]] # i
#     # print(np.where((a2rr[:-1] == 0x00))[0])
#     # print(np.where((a2rr[1:] == 0x55))[0])
#     # array_r = np.zeros((len(start), 16), dtype=np.uint8)
#     array_r = np.zeros((len(start), 4, 4), dtype=np.uint8)
#     print(start)
#     # print(start2)
#     k=0
#     # for i in range(12):
#     #     if i % 3 == 0 and i > 0:
#     #         k +=1 
#     #     array_r[:, i+k] = a2rr[np.add(start, i)]
#         # array_r[:, i, :-1] = a2rr[np.add(start, i)]
#     for i in range(4):
#         for j in range(3):
#             array_r[:, i, j] = a2rr[np.add(start, k)]
#             k += 1
#     # print(array_r)
...
#     # matr = np.einsum("j,i->", array_r, a)
#     # matr = np.einsum("ijk,k->ij", array_r, a)
#     # matr = np.einsum("ikj,k->ijk", array_r, a).astype(np.int32)
#     # b = b.transpose
#     # matr = (np.einsum("ijk,jk->ji", array_r, b) / 256).astype(np.int32)
#     matrix[:,1:] = (np.einsum("ijk,jk->ij", array_r, b) / 256).astype(np.int32)
#     # matr = np.array(np.einsum("ikj,jk->ij", array_r, b) / 256, dtype=np.int32)
#     # matr = np.einsum("ijk,jk->ij", array_r, b).astype(np.int32)
#     # print(matrix)
...
class MainWindow(QWidget):
    ...
    def saveData(self):
        data = textedit.toPlainText().split('\n')
        
        save_thread = SaveThread(data)
        save_thread.finished.connect(saveFinished)
        save_thread.start()
