...
float arr[9];
float temp = 0; /////////////////////////////////////////////////////////////////////////////////////////////////////////////
float median_enc_angle = 0;
float median_enc_old = 0;
/******************************************************************************/
...
//
	*pTM0PRD = 80000;    //200Hz  //27000	 29600  // 80000 - 1 кГц //////////////////////////////////////////////////////////////// !!!!!!!!!!!!!!!!!!!
     ...
     Cmd =  ((packin[0] | (packin[1] << 8)) << 16) >> 16; // (16-bits word with sign) 
      ...
      if (Cmd == 0) {PwMotor = 0; flag_STOP = true; 
      	...
      	integral_amp = 0; K_amp_vel = 1; // !
                                     ...
                                     // Find Zero position
      else if (Cmd == 99) {flag_find_Zero = true; flag_STOP = false;}  
                                     // Find Right position
      else if (Cmd == 100) {flag_find_Right = true; flag_STOP = false;} 
                                     // Start vibration
      else {
      	if (Cmd == 77) 
       {
     	...
     	//integral_amp = 0; K_amp_vel = 1; // 
		  K_amp_vel = 0.9 * (((-4.7536811e-09)*powf(FrqVibr, 4) +
                              (2.0656983e-06)*powf(FrqVibr, 3) +
                              (1.6e-04)*powf(FrqVibr, 2) +
                              1.0e-03*FrqVibr)) + 0.941254199; 
		//if (K_amp_vel > 35) {K_amp_vel = 35;}
		//iVibro = 0; ///////////////////////////////////////////////////////////////////////
...
//	++++++++++++++ MEMS-Gyro parameters ++++++++++++++++++++++++++++		
 		...
 		bufout[0] = 0x72;
   	    bufout[10] = (((int)(Vel_Target*1000) >>16) & 0xff); // 1
		bufout[11] = (((int)(Vel_Target*1000) >>8) & 0xff);      
		bufout[12] = (((int)(Vel_Target*1000)) & 0xff);            

		   bufout[1] = (((int)(median_enc_angle*100) >>16) & 0xff);     // 4	
           bufout[2] = (((int)(median_enc_angle*100) >>8) & 0xff);  
	       bufout[3] = (((int)(median_enc_angle*100)) & 0xff);		

	     bufout[4] = (((int)(vel_angle*100) >>16) & 0xff);     // 2
         bufout[5] = (((int)(vel_angle*100) >>8) & 0xff);  
	     bufout[6] = (((int)(vel_angle*100)) & 0xff);  
	     // если прореживание переменное, то надо делить на него и домножать на 10000, чтобы числа не менялись,
	     // иначе на стенд идут меньшие значения
	// vibro
		bufout[7] = (0); // 1
		bufout[8] = ((iVibro >>8) & 0xff);      
		bufout[9] = ((iVibro) & 0xff); 
		//	
...
{    // setting PWM parameters ----------------------------------------
    *pPWMPERIOD1 = 4000; //  Freq = Fperif/N = 160000000/2000 = (40KHz)  менять ли?
	*pPWMAL1 = (unsigned int)(-1999);   // 
	*pPWMDT1 = 700;
//      
//===========================================================================
    *pPWMPERIOD2 = 4000; //(40kHz)  2000; //  Freq = 160000000/2000 = (80KHz)  менять ли?
...
{	 	
	...
	int periods = 0; // зачем каждый раз создавать, если используем в 1 из 10?
//-------------------------------------------------------------
        cou_div++;
//         
         enc_angl_gr = 5729.578*enc_angl; //  enc_angl - это то, что мы получу с энкодера
         // именно enc_angl пересчитывается в motor_angl для формирования управления
         // получается, децимация не нужна для подачи управления? понять это
//        enc_angl_gr точно не 4 или 9 раз суммируется?
		if (flag_vibration) ////////////////////////////////////////////////////////////////////
		{
		//FrqNmb = (int) (roundf(40000 /(float) FrqVibr)); 
		FrqNmb = (int) ((40000 /(float) FrqVibr) + 0.5); // лучше еще и округлением, не нужно каждый раз
		// 0.5 для округления 
		iVibro++;
			if (iVibro > FrqNmb) // должно помочь
			{
				err_amp = (SetVel*10 - max_vel)/(SetVel*10);
         ...
         if (cou_div > 9) // || (iVibro > FrqNmb)) //event frequency 4000 Hz
          {  

			
          	// enc_angl_gr текущее выводить, хочу оценить уровень шума при задании частоты
          	// может, перейти к посылке 20 байт, чтобы больше данных выводить на графики !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      	  	 cou_div = 0;     
          	 motor_angl = 13*enc_angl; 
          	 int periods = 0; // лучше здесь создать один раз, чем вне условия 10
          	 periods = 0.5*motor_angl/pi;
          	 motor_angl = motor_angl - periods*2*pi;
// --  ----------------------------------------------------------------------------------------          	 
         	 if (cou_init < 3000) 	// +++++ start initialization
          	  {                     // One time !
          	 	cou_init++;
          	 	motorCTL(400, 0.0); //was 400 => 0.2*POWERmax (go to nearest pole zero position)
          	 	// частота 0, амплитуда 400
          	 	N_DUperiod = 0;
...
// -- PI  regulator    -----      Amplitude Stabilization  -------------------------------------
			 if (flag_vibration)  // Start vibration 
			  {                   
				if (vel_angle > max_vel) {max_vel = vel_angle;} // можно искать среднее между минимумом и максимумом
...
// -- PI  regulator    -----      Velocity Stabilization   ------------------------------------
          	  	 err_vel = (float)(Vel_Target*10*K_amp_vel - vel_angle); // resolution 0.1 degree
          	  	 if (flag_STOP) integ_angl = 0;
          	  	 else integ_angl += 0.009*err_vel;  // was 0.017
          	  	                             
          	  	 if (integ_angl > 1900) integ_angl = 1900;
          	  	 else {if (integ_angl < -1900) integ_angl = -1900; }
...
// --------------------------------------------------------------------------------------------   
          	  	  ...
          	  	  else {if (PwMotor < -1990) PwMotor = -1990; } 
          	       
// ****************************************************************************************************         	  	 
                  if ((flag_STOP) || (enc_angl_gr > 18000) ||(enc_angl_gr < -18000))   PwMotor = 0;            // Emergency STOP Rotation	!!!!!!!!!!!!!!! 
                  //if (enc_angl_gr > 18000) PwMotor = 0;    // Limitation +180 dgr
                  //else {if  PwMotor = 0;}   // Limitatiom -180 dgr
// ****************************************************************************************************  
...
//                 				   
          	median_enc = arr[4] * 9;
          	if ((median_enc > arr[3] * 9) || (median_enc < arr[5] * 9)) median_enc = -1;
          	int i;
			for ( i=0; i < 9; i++) {arr[i] = -99999999;}         ////////////////////////////////////////////////////////////////////////////////////////////////////////////
			median_enc_angle = 40*(median_enc - median_enc_old);
   			median_enc_old = median_enc;
            }  // end of decimation
           else {
           	     angle_sm += enc_angl_gr; // точно не 9 раз сработает вместо 10?
           	     ////////////////////////////////////////////////////////////////////////////////////////////////////////////
           	     if (cou_div < 10)
           	     {
	           	     arr[9 - cou_div] = enc_angl_gr;
	           	     int i;
	           	     for(i=0; i < 9; i++)
	           	     {
	           	     	if (arr[i] > arr[i+1])
